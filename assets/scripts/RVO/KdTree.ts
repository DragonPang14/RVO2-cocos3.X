...
export default class KdTree
{
    ...
    private buildObstacleTreeRecursive(obstacles: Array<Obstacle>): any
    {
        ...
        for (let j = 0; j < obstacles.length; ++j)
        {
            ...
            {
                let t = RVOMath.det(Vec2.subtract(obstacleI2.point_, obstacleI1.point_), Vec2.subtract(obstacleJ1.point_, obstacleI1.point_)) / RVOMath.det(Vec2.subtract(obstacleI2.point_, obstacleI1.point_), Vec2.subtract(obstacleJ1.point_, obstacleJ2.point_));
                let splitPoint = Vec2.addition(obstacleJ1.point_, Vec2.multiply2(t, (Vec2.subtract(obstacleJ2.point_, obstacleJ1.point_))));
    ...
    private queryObstacleTreeRecursive(agent: Agent, rangeSq: number, node: ObstacleTreeNode)
    {
        ...
        let distSqLine = RVOMath.sqr(agentLeftOfLine) / RVOMath.absSq(Vec2.subtract(obstacle2.point_, obstacle1.point_));
